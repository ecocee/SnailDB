 ...
 * 
 * NOTE: These tests require a SNAILDB server running on localhost:12222
 * If the server is not available, all tests will be skipped
 */
...
describe('SNAILDB Client Integration Tests', () => {
  ...
  beforeAll(async () => {
    ...
    try {
      ...
      console.log('✅ Connected to SNAILDB server');
    } catch (error) {
      serverAvailable = false;
      connectionError = error as Error;
      console.warn('⚠️  SNAILDB server not available - all tests will be skipped');
    }
  });
  ...
  /**
   * Skip test suite if server is not available
   */
  if (!serverAvailable) {
    it('⚠️  SNAILDB server not running - skipping all tests', () => {
      expect(true).toBe(true); // Placeholder test
    });
  } else {
    // All tests only run if server is available
